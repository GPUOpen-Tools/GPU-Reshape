...
namespace Passes 
{
	namespace Basic
	{
		enum class ExportStabilityValidationModel : uint8_t
		{
			eFragment,
			eCount
		};

		enum class ExportStabilityValidationErrorType : uint8_t
		{
			eNaN = (1 << 0),
			eInf = (1 << 1),
			eMax = (eNaN | eInf)
		...
		class ExportStabilityPass : public IDiagnosticPass
		{
		public:
			ExportStabilityPass(DeviceDispatchTable* table, DeviceStateTable* state);
		...
		private:
			 ...
			 */
			void InsertBatched(SStringCache *message_cache, SCommandBufferVersion &version, uint64_t key, const SDiagnosticMessageData& message, uint32_t count);
